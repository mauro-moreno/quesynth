...
filter_distortion_patch :: proc(
	filter_type, cutoff, resonance, gain: int,
	saturation := 0,
	shape := 0,
	width := 64,
) -> cpatch.Patch {
...
filter_fundamental_amplitude :: proc(audio: []f32, f0: f64) -> f64 {
	...
	// A Hann window and one-sided sinusoid each contribute a factor of two.
	return 4.0 * math.sqrt(re * re + im * im) / n
}

cmd_filtersaturation :: proc(
	dll: string,
	filter_type, cutoff, resonance, shape, width: int,
	note: u8,
	saturation_spec, gain_spec: string,
) {
	...
	dumped := false
	dump_indices := []int{0, 5, 8, 14, 19, 20, 21, 22, 23, 29}

	fmt.printfln("filtersaturation: type %v, cutoff %v, resonance %v, note %v (%v Hz), shape %v, width %v",
		filter_type, cutoff, resonance, note, dec1(f0), shape, width)
	fmt.println("  selected oscillator through an open, non-moving filter; each row is normalised later")
	fmt.println("  against saturation 0 at the same input gain")
	fmt.println()
	fmt.printfln("%6v %6v %10v %10v %10v %10v %10v %10v %10v %10v %10v %10v",
		"gain", "sat", "ref THD", "our THD", "ref fund", "our fund", "ref amp", "our amp",
		"ref RMS", "our RMS", "ref peak", "our peak")

	for gain in gains {
		for saturation in saturations {
			p := filter_distortion_patch(filter_type, cutoff, resonance, gain, saturation, shape, width)
			...
			ref_amp := filter_fundamental_amplitude(ref_audio, f0)
			our_amp := filter_fundamental_amplitude(our_audio, f0)
			...
			fmt.printfln("%6v %6v %10v %10v %10v %10v %10v %10v %10v %10v %10v %10v",
				...
				dec5(ref_amp), dec5(our_amp), dec4(ref_rms), dec4(our_rms), dec4(ref_peak), dec4(our_peak))
